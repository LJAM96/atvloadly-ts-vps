...
1. 如果需要自动 Bonjour 发现，Linux/OpenWrt 宿主机需要安装 `avahi-deamon` 服务
...
2. 请参考下面的命令进行安装，记得修改下 mount 目录
   ...
   **Docker:**
   ...
   当启用 Bonjour 发现时，宿主机的 `/var/run/dbus` 和`/var/run/avahi-daemon` 需要共享给 docker 容器使用
...
如果 Bonjour 发现不可用，也可以在 `偏好设置 -> 实验室` 中使用 Pair 文件直连：

1. 使用 `idevice_pair` 导出 Pair 文件
1. 填写 Apple TV 的设备 IP（例如 Tailscale IP）
1. 上传并导入 Pair 文件
1. atvloadly 会回退到 usbmuxd 轮询模式，即使没有 Avahi 发现也能显示已配对设备
...
> 请关闭 VPN，并重启 AppleTV，重新进入配对模式，确保在[**工具**]菜单中能发现`_apple-pairable._tcp`类型的设备，并重新配对连接

> 如果你已经有 Pair 文件，也可以进入 **偏好设置 -> 实验室**，直接填写 Apple TV IP 后导入。
